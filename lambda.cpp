...
/* multiply matrix -----------------------------------------------------------*/
void
matmul(const char *tr, int n, int k, int m, double alpha, const double *A, const double *B, double beta, double *C) {
	double d;
...
static int LD(int n, const double *Q, double *L, double *D)   //此处分解算法同 FMFAC5（参见"模糊度经典版.pdf"）
{
	...
	for (i = n - 1; i >= 0; i--) {          //FMFAC5中是从i=n开始的，为何有区别？由于矩阵元素是从0到n*n-1，而FMFAC5中是1到n*n，故此处需要多减1
		if ((D[i] = A[i + i * n]) <= 0.0) {
			info = -1;
			break;
		}  //D(i,i)=Q(i,i),注意此处D是一个列向量而非矩阵
		a = sqrt(D[i]);                                //a=sqrt(Q(i,i))
		for (j = 0; j <= i; j++) L[i + j * n] = A[i + j * n] / a;      //L(i,1:i)=Q(i,1:i)/sqrt(Q(i,i))
		for (j = 0; j <= i - 1; j++)
			for (k = 0; k <= j; k++)
				A[j + k * n] -= L[i + k * n] * L[i + j * n];  //Q(j,1:j)=Q(j,1:j)-L(i,1:j)L(i,j) 同样的由于矩阵原因，j从0开始
...
{                                                              //我认为该算法与SRC1只是看起来不同，但运行过程是相同的
	...
	j = n - 2;
	k = n - 2;    //此处k指i1,由于矩阵原因变为n-2   j指i,同样的由于矩阵原因变为n-2
	while (j >= 0) {   //while i>1
		...
		if (del + 1E-6 < D[j + 1]) { /* compared considering numerical error */
			perm(n, L, D, j, del, Z);
			k = j;
			j = n - 2;   //即i1=i，i赋初值以便进行下一次循环。（相比于SRC1算法少了sw参数并且少了sw那一层循环）
		}
		else j--;
	}
}
...
static int search(int n, int m, const double *L, const double *D,
	const double *zs, double *zn,
	double *s) {                                                            //n:float parameters(nb=16)   m: fixed solutions(=2)   zs:即z=Z'*a
...
/* ambiguity fix ---------------------------------------------------------------
 * try fix amb, float to integer
 * args     :   Ref<VectorXd> amb               IO
 *              const Ref<const MatrixXd> Qaa   I
 * ---------------------------------------------------------------------------*/
double amb_fix(const Ref<const VectorXd> amb_float, const Ref<const MatrixXd> Qaa, Ref<VectorXd> amb_fixed) {
	int na = amb_float.rows();
	double *a = mat(na, 1);
	double *Q = mat(na, na);
	double *F = mat(na, 2);
	double *s = mat(1, 2);
